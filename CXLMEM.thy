...
fun external_step :: "(cxl_state * Memop list * Memop_res list)  => (cxl_state * Memop list * Memop_res list)"
  where
    "external_step (xst, (Read i # mops), mress) = 
    (xst \<lparr>counter := counter xst + 1 \<rparr> \<lparr>m2sreqs := MemRd (counter xst) i # m2sreqs xst \<rparr>, mops, Pending (counter xst) (Read i) # mress)"
  | "external_step (xst, (Write i v # mops), mress) = 
    (xst \<lparr>counter := counter xst + 1\<rparr> \<lparr>m2srwds := MemWrite (counter xst) i v # m2srwds xst \<rparr>, mops, Pending (counter xst) (Write i v) # mress)"
