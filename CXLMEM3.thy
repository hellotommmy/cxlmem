...
begin
(* Executable model: memory as finite association list *)
...
value "initial1 \<leadsto>e next1_external = True"

lemma initial1_to_next1_external: "initial1 \<leadsto>e next1_external"
  unfolding initial1_def next1_external_def mem3_42_def
  by (rule external_step.intros(1))

(* Make the relations executable *)
code_pred external_step .
code_pred internal_step .
code_pred system_step .

(* Enumerate next states from initial1 *)
values "{s. initial1 \<leadsto> s}"
values "{s. initial1 \<leadsto>e s}"
values "{s. initial1 \<leadsto>i s}"
